...
### Fixed
- AI-generated test steps no longer display literal HTML entities (e.g. `&#39;` for an apostrophe, `&amp;` for `&`). Step **action** and **expected result** are plain-text fields rendered as plain text, but the generation, regeneration, and draft-edit sanitizers left them HTML-escaped (unlike name and category, which were already unescaped). They are now unescaped back to plain text after sanitizing; literal markup is still stripped, and `source_refs` stay strictly sanitized. Existing drafts generated before this fix keep the old escaped text until regenerated.
