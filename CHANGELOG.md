...
### Fixed
- **Opening a test case no longer loads its side panels twice.** The Latest Executions list, the Activity timeline and the Reqs / Linked Defects tab counts each flashed back to a loading state a moment after they had already rendered. Both loaders resolved the test id as `test?.id || testId` but listed each source separately in their dependency arrays, so when the fetched test arrived and `test?.id` caught up to the route parameter the effect saw a dependency change and re-ran with the very same id — a second, redundant round trip per panel. The version/requirement/defect loader also cleared its state at the start of that second run, which is why the Activity timeline and those two tab counts blanked out and repopulated. Both effects now depend on a single derived id, and each retires its own response if a newer one supersedes it, so switching between test cases quickly can no longer land an older response over a newer one.
- **Category pickers and filters now list every category, not just the first ten.** The test grid, the run list and the test-case detail panel each requested categories without paging arguments and so took the endpoint's default page size, silently hiding every category past the tenth from selection and filtering on any instance with more than ten. All category pickers now read one shared, unpaginated copy of the list, which also removes the duplicate requests those independent fetches produced — opening a test case issued six identical category requests where the app now makes one in total, refreshed when categories are created or deleted. The Category Management screen keeps its own paginated fetch, since it needs page and search parameters.
- **Comment, version-history and linked-requirement panels no longer risk showing another record's data.** All three are reused in place when the surrounding detail view switches records, and their fetches had no way to retire a superseded response, so an older request resolving last would overwrite the current one. Comments and version history are additionally refetched imperatively — after adding, editing or deleting a comment, and on history retry or restore — which could race the same panel's own reload. Each now discards responses that a newer request has superseded.
- **Concurrent result writes for the same test case no longer fail with a 500.** `POST /api/runs/{id}/results` picked the next `attempt_number` with an unsynchronized read-then-insert, so parallel writers read the same `MAX(attempt_number)` and all but one lost the UNIQUE index race — surfacing as an opaque `{"error":"internal server error"}` with no hint that a retry would succeed. This is exactly the documented CI path: parallel shards and automatic retries reporting one test case at once (measured on a live instance: 12 concurrent writes → 7 created, 5 failed). The read and the insert now share one transaction that opens with the parent run's `updated_at` bump, so it takes SQLite's write lock from its first statement and concurrent callers queue on `_busy_timeout` instead of deadlocking on a read-to-write upgrade, which SQLite rejects immediately regardless of that timeout. That leading write's `RowsAffected` also replaces the separate parent-run existence check, closing the window where a run deleted mid-call left an orphan result. Covered by a regression test firing 12 simultaneous writes and asserting attempts land as 1..12 with no gaps or duplicates (file-backed, since `:memory:` gives every pooled connection its own database and could never contend).
...
- **`GET /api/runs/{id}/analysis-job` no longer 404s when the run simply has not been AI-analyzed.** The run-detail page mounts a progress banner that polls this endpoint on every open, so any run that had never been through failure analysis — the overwhelming majority, including every run on the public demo stand — logged a red 404 in the browser console, which reads as a broken feature while nothing is actually wrong. Worse, the handler never checked the run at all, so a genuinely bad run id and a valid run with no job yet returned the identical 404 and the status code could not tell "wrong URL" from "nothing yet". The endpoint now verifies the run exists and 404s only for a missing run, returning `200 null` when there is no job — matching `GET /api/runs/{id}/analyses/current`, which already answers `200 {}` for the same empty case. The banner needs no change: it already treated a missing job as "render nothing".
...
- **Break-glass password recovery: a `reset-password` server subcommand, plus "Forgot password?" guidance on the sign-in screen.** Until now a sole administrator who forgot their password was hard locked out: there is no reset-email flow (by design — TTGO has no SMTP), `ADMIN_EMAIL`/`ADMIN_PASSWORD` seeding refuses to touch an instance that already has an active admin, and the `ttgo` CLI authenticates through the API — so the only way back in was hand-editing the SQLite file with a self-made bcrypt hash. The server binary now doubles as the recovery tool: `reset-password <email>` runs on the server host against the same `DB_PATH`/`.env` the server uses (shell access to the host *is* the authorization — no login involved), prints a generated temporary password, re-enables the account if it was deactivated or soft-deleted, revokes all of its sessions, and records a `user.password_reset` audit entry marked `method=break_glass_cli`. It is safe to run while the server is up (WAL journal + busy timeout) and the new password works immediately, with no restart. A mistyped email fails with the instance's administrator emails listed instead of leaving the operator guessing, and a wrong working directory reports where it looked for the database rather than silently creating an empty one. The sign-in screen documents the dead end where it happens: a "Forgot password?" expander explains the normal path (any admin, via Settings → Users → Reset Password) and, for when no admin can sign in, shows the exact break-glass command pre-filled with the typed email.
...
- **Defects can be assigned to an owner, marked `fixed`, and updated in bulk.** `assignee_id` is now accepted on defect create, update and the create-and-link path used when filing a defect from a run result — an id assigns, `""` clears, and an absent field leaves the owner alone. The value is validated in the shared `ValidateCreate`, so every path that files a defect rejects an unknown, deactivated or deleted user with a 400 rather than storing a dangling reference, and the list and detail responses carry a resolved `assignee_name` (display name, else email) from a batch user lookup that still names an owner whose account was since deactivated. `status` gains a third stored value, `fixed`, meaning *fixed but not yet retested*; `"open"` stays fully valid and writable, so the CLI, seed data, webhooks and any external script keep working unchanged, and all four defect counters (per run result, per run, per test case, per folder) already bucketed `closed` against everything else, so a `fixed` defect counts as open with no code change. The new `POST /api/defects/bulk-update` applies one set of `status` / `severity` / `assignee_id` values across up to 500 ids in a single grouped write, tolerating ids that no longer exist and returning the updated rows fully enriched; a request that names ids but sets no field is a deliberate read-back that writes nothing, because `updated_at` is what the register reads as a staleness signal. `GET /api/defects/{id}/tests` now also reports, per affected test case, the run it **last failed** in (`last_run_id`, `last_run_name`, `last_result_status`) — the most recent `FAIL`/`ERROR` result for that test case, not merely whichever historical result the defect happened to be filed against, since a test-case-scoped link carries no result at all.

### Changed
- **In-app help gains a Defects section.** Defects is a top-level page in the Quality workspace and had no help of its own — it was mentioned only in passing under Test Runs and AI Failure Analysis — while having the most behaviour worth explaining of any page: four displayed statuses derived from three stored ones, tiles that double as filters, bulk actions, and a Retest that builds a run from a defect's affected tests. All of that is now written down where someone looking at the page can find it.
- **The demo dataset now exercises the defects triage queue.** Every seeded defect was filed open and unassigned, so a fresh install — and the public demo — opened the register with an all-Unassigned Owner column and three of its four triage tiles reading zero, which reads as dead controls rather than an empty queue. The seed now covers each one: open and unassigned (and critical), open and owned, fixed and owned, and closed, with one backdated so the staleness tile has something in it. The fixed defect deliberately shares a test case with a still-open one, so reverification stays honest.
- **Every dialog in the app now contains keyboard focus.** `role="dialog"` and `aria-modal` promise the page behind is unreachable, but Tab walked straight out of every overlay into whatever it was covering. The shared shell behind the import, resync, screenshot-gallery and requirements dialogs now traps Tab and hands focus back to whatever opened it, matching the confirm and defect dialogs.
- **The Defects register is usable at phone widths.** Below 640px the triage tiles stack and the filter bar wraps, so the page's fixed chrome grew past the viewport and squeezed the queue itself down to a single pixel — the rows existed but there was nothing to scroll to reach them. The page now grows and scrolls as a whole at that width, and the bulk bar sticks to the viewport instead of the foot of the document.
- **The Defects register is now a triage queue rather than a record table.** The page answered "what exists" — `Total / Open / Closed` counts over card rows tall enough that roughly nine fit on a 1080px screen, with no owner, no age, no sorting and no bulk action, and a status pill that was a *button*: one mis-click silently closed a live defect with no undo. It now opens on four triage tiles (Needs triage, Critical open, Stale · 7d+, Fixed awaiting retest), a filter bar with status tabs, multi-select severity chips and a sort control (severity-then-oldest / recently updated / tests affected), and a 36px hairline table that fits twenty rows under a sticky header, with a 3px severity edge bar that reads before any text does. `open` was three queues wearing one label, so the queue displays four statuses derived from two stored facts: a stored `open` defect splits into **Needs triage** or **In progress** on whether it has an owner — assignment *is* the triage action — while `fixed` and `closed` display as themselves. The status pill is now static text; nothing on a row writes on click. Selecting rows reveals a bulk bar (assign, unassign, set severity, mark verified & close) that applies the whole selection in one request and reports a failure inline instead of silently doing nothing, and any change to the search, tab, chips or sort clears the selection so a bulk close can never land on rows scrolled out of the filter — as does editing a defect out of the queue it was ticked in, the one other way a selected row can leave the view. While an apply is in flight every checkbox on the table freezes for the round-trip — the id set the call captured must not move under it — and each row's own Open detail / Delete / Retest actions freeze on exactly the rows that call captured, since the edit modal is a full-record editor seeded from the row as it was *before* the bulk write, so a save landing after it would PATCH the pre-bulk status and severity straight back over the change and silently revert it for that row. That second freeze follows the captured ids rather than whatever is ticked at the time: the filter controls stay live during an apply and every one of them clears the selection by design, so a re-sort or a keystroke mid-flight would otherwise have handed those actions straight back on the rows still being written to. Rows the call is not writing to are untouched and stay fully usable. A defect that somebody else deleted since the page loaded also drops off the queue when the apply comes back without it, rather than sitting there unchanged and telling the user they just closed something that no longer exists. Expanding a row lists the tests the defect affects, each linking to the run it last failed in, plus the description, Open detail, Delete, and a **Retest** that builds a run from exactly those tests. The whole list is fetched once on mount and filtered, sorted, searched and counted in memory, which removes the request-per-keystroke and the blank "Loading…" table the old page showed on every filter change, and lets the tiles keep counting the full backlog while the table shows a slice; a comment marks ~5k defects as the point to revisit with a server-side list plus a stats endpoint. `?focus=<id>` still deep-links a defect — it now resets the filters, expands and scrolls to the row instead of opening the modal, and the parameter is no longer stripped from the URL. The delete confirmation is a real dialog naming the defect and warning that its links go with it, replacing the last `window.confirm` on the page. Both themes are token-only (no hardcoded colours), all four tiles plus the tabs and chips are real buttons carrying `aria-pressed`, and Ctrl/⌘-K focuses the search box (except while a dialog is open, so it cannot steal focus out of a form someone is typing in). Every tile lands on exactly the rows it counted: alongside query, status and severity, the client-side filter carries a "not closed" and a "no update in 7 days" dimension, which is what Critical open and Stale · 7d+ need and what neither a status tab nor a severity chip can express. Those two dimensions are set only by a tile and cleared the moment a tab or chip is touched, so the table can never end up narrower than the page says with nothing on screen to explain why. The tiles are toggles, as their `aria-pressed` advertises: clicking a pressed one releases its filter and returns the whole queue — for Stale · 7d+ that is the only control on the page that reflects the filter at all, since nothing in the filter bar can express it. The counts themselves are still taken over the *whole* backlog, not the filtered view — that is the point of a tile — so with a search term typed, or a severity chip on, the tiles and tab counts deliberately describe more than the table shows.
- **The defect edit modal gains an owner, and offers `fixed` as a status.** It is the only per-defect way to set an assignee: a picker listing the assignable users plus "Unassigned", which also keeps the current owner in the list when they are no longer assignable (marked *(inactive)*), so saving an unrelated field cannot silently drop them — and an assignee that was not touched is left out of the request entirely, so a defect owned by a since-deactivated user is still editable rather than failing validation on the way out. The status select now offers Open / Fixed (awaiting retest) / Closed instead of the raw lower-case stored values.
- The analytics **Unique Bugs** table and the linked-bug panels on a test case now treat any defect that is not `closed` as active, so a `fixed` defect is still counted and still shown as outstanding; the Unique Bugs status badge gained a `Fixed` entry, which previously fell through to a badge labelled "Open".
- `test_cases.reverification_flagged` now means *every linked defect is fixed-or-closed*, i.e. the test is ready to be retested, instead of *every linked defect is closed*. The flag was already the codebase's "needs retesting" signal, so `fixed` joins it rather than competing with it, and it now fires at the moment QA wants it — before the defect is closed, not after. Because `fixed` is a brand-new value no existing row changes behaviour, and bulk-closing defects recomputes the flag over the same test cases that closing them one at a time would.
