...
### Added
- **AI failure-analysis verdicts now suggest a defect type, and their accuracy is measured.** An untriaged failing result whose analysis produced a usable verdict shows a compact `AI: <type>` chip with a one-click **Accept** beside the defect-type control — Accept applies the suggested type, overriding is just the existing dropdown, and the chip disappears once the result is triaged. Nothing is written automatically; the human's decision stays the record. At that decision moment the backend snapshots what the AI had suggested onto the result (`suggested_verdict` / `suggested_defect_type` / `suggested_confidence`), so agreement is a direct comparison of stored columns and is immune to a later re-analysis changing the verdict. Snapshotting covers both the single-result and bulk triage paths and is best-effort — a missing or failing analysis lookup logs and skips it, never blocking a human's triage write. A new `GET /api/ai/failure-analysis/accuracy?days=30` endpoint and an accuracy panel in AI Failure Analysis settings report the agreement rate overall, by verdict, and by confidence; the by-confidence ladder is the point — a clean descent means the model's confidence is trustworthy, a flat one means it is noise. Results still sitting at the `to_investigate` auto-default (i.e. untriaged) and results that carried no suggestion are excluded from the stats rather than counted as disagreements.
