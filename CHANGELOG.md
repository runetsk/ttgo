...
### Fixed
- AI-generated test steps no longer display literal HTML entities (e.g. `&#39;` for an apostrophe, `&amp;` for `&`) in the AI Generate review — the compare/diff modal, step list, and draft editor. Step text is now stored as sanitized HTML consistently (matching manually-authored and imported test cases, which are rendered via the HTML-decoding step views), and the plain-text AI-draft views decode entities for display. This replaces an earlier interim approach that unescaped step text in the backend, which was inconsistent with the other paths and could push a literal `<`/`>` from an accepted draft into the HTML-rendered run views.
