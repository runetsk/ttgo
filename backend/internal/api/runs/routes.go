...
func Mount(api *routegroup.Bundle, h *Handler, requireAuth routing.AuthMiddleware) {
	...
	api.HandleFunc("GET /runs/{id}/defect-links", requireAuth("read", h.ListRunDefects))
	api.HandleFunc("GET /runs/{id}/results/{result_id}/defect-links", requireAuth("read", h.ListResultDefects))
	api.HandleFunc("POST /runs/{id}/results/{result_id}/defect-links", requireAuth("write", h.LinkResultDefect))
	api.HandleFunc("POST /runs/{id}/results/{result_id}/defects", requireAuth("write", h.CreateAndLinkResultDefect))
	api.HandleFunc("DELETE /runs/{id}/results/{result_id}/defect-links/{defect_id}", requireAuth("write", h.UnlinkResultDefect))
}
